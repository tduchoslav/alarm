/**
 * @file MyESP8266Mini.h
  tomas duchoslav - minimalized version saving memory
 */
...
class MyESP8266Mini {
 public:
     ...
     */
    MyESP8266Mini(SoftwareSerial &uart, uint32_t baud = 9600);
#else /* HardwareSerial */
    /*
     * Constuctor. 
     *
     ...
     * @warning parameter baud depends on the AT firmware. 9600 is an common value. 
     */
    MyESP8266Mini(HardwareSerial &uart, uint32_t baud = 9600);
#endif
    ...
    int httpGet(char* request, String serverName, int port);
